...
public sealed class StorageService
{
    ...
    private const int MaxFileNameDotCount = 2;
    ...
    // Ensures a container exists with the given properties; used at startup to
    // provision the runtime-utils containers in-process.
    public async Task<ContainerDbEntry> EnsureContainerAsync(string name, string owner, bool isPublic, TimeSpan retentionPeriod, CancellationToken cancellationToken = default)
    {
        ArgumentOutOfRangeException.ThrowIfLessThan(retentionPeriod.TotalSeconds, 1);
        ArgumentException.ThrowIfNullOrWhiteSpace(owner);

        if (!ValidateContainerName(name))
        {
            throw new ArgumentException("Invalid container name", nameof(name));
        }
        ...
        var entry = new ContainerDbEntry
        {
            Name = name,
            Owner = owner,
            IsPublic = isPublic,
        ...
        dbContext.Containers.Add(entry);
        ...
        {
            existing = await dbContext.Containers.AsNoTracking()
                .FirstOrDefaultAsync(c => c.Name == name, cancellationToken);
            if (existing is not null)
            {
                return existing;
    ...
    public async Task<ContainerDbEntry?> GetContainerAsync(string container, CancellationToken cancellationToken = default)
    {
        await using var dbContext = _db.CreateDbContext();

        return await dbContext.Containers.AsNoTracking()
            .FirstOrDefaultAsync(c => c.Name == container, cancellationToken);
    }

    public async Task<bool> ExistsAsync(string container, string path, CancellationToken cancellationToken = default)
    {
        if (!ValidateContainerName(container) || !ValidateFilePath(path))
        {
            return false;
        }
        ...
        bool exists = await dbContext.Files.AsNoTracking()
            .AnyAsync(f => f.Id == id, cancellationToken);

        return exists && File.Exists(fullPath);
    }

    public async Task<FileDbEntry?> GetFileEntryAsync(string container, string path, CancellationToken cancellationToken = default)
    {
        if (!ValidateContainerName(container) || !ValidateFilePath(path))
        {
            return null;
        ...
        GetFullPath(container, path, out string id);

        return await dbContext.Files.AsNoTracking()
            .FirstOrDefaultAsync(f => f.Id == id, cancellationToken);
    }

    public bool TryGetLocalFilePath(string container, string path, [NotNullWhen(true)] out string? fullPath)
    {
        if (!ValidateContainerName(container) || !ValidateFilePath(path))
        {
            fullPath = null;
        ...
        fullPath = GetFullPath(container, path, out _);
    ...
    // Server-side upload. Upserts: existing file with the same path is replaced.
    public async Task UploadAsync(string container, string path, Stream stream, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await using (FileStream fs = File.Open(tempPath, new FileStreamOptions
            {
                ...
                PreallocationSize = stream.CanSeek ? stream.Length : 0,
            ...
            DateTime now = DateTime.UtcNow;
            long contentLength = new FileInfo(fullPath).Length;

            await dbContext.Files
                .Where(f => f.Id == id)
                .ExecuteDeleteAsync(CancellationToken.None);

            dbContext.Files.Add(new FileDbEntry
            {
                Id = id,
                Path = path,
    ...
    public async Task<IResult> DownloadFileAsync(HttpContext context, string container, string path)
    {
        ...
        if (File.Exists(fullPath))
        {
            string? contentType = MediaTypeMap.GetMediaType(Path.GetExtension(path));
